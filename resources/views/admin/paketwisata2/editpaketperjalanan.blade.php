...
@section('container')
    <div class="max-w-3xl mx-auto py-12 px-6">
        <h1 class="text-2xl font-bold text-gray-800 mb-6">Edit Paket Wisata Perjalanan</h1>

        <form action="{{ route('paketwisata2.update', $paket->id) }}" method="POST" class="space-y-8">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select name="label" required class="w-full border rounded-lg p-3 mt-1">
                    @foreach ($availableLabels as $label)
                        <option value="{{ $label }}" {{ $paket->label == $label ? 'selected' : '' }}>
                            {{ $label }}
                        </option>
                    @endforeach
                </select>
                ...
                <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                    <table class="w-full text-sm text-left text-gray-500">
                        ...
                        <tbody id="itinerary-table">
                            @foreach ($paket->itineraries as $i => $itinerary)
                                <tr class="bg-white border-b">
                                    <td class="px-6 py-4">
                                        <input type="text" name="itinerary[{{ $i }}][hari]"
                                            value="{{ $itinerary->hari }}" class="w-full border rounded-lg p-2">
                                    </td>
                                    <td class="px-6 py-4">
                                        <input type="text" name="itinerary[{{ $i }}][waktu]"
                                            value="{{ $itinerary->waktu }}" class="w-full border rounded-lg p-2">
                                    </td>
                                    <td class="px-6 py-4">
                                        <textarea name="itinerary[{{ $i }}][deskripsi]" rows="2" class="w-full border rounded-lg p-2">{{ $itinerary->deskripsi }}</textarea>
                                    </td>
    ...
    {{-- Script tambah/hapus baris itinerary --}}
    <script>
        let rowIndex = {{ count($paket->itineraries) }};
